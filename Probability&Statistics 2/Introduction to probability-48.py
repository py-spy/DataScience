...
print(flags[:2])
sorted_bars = flags.sort_values('bars', ascending = [0])
most_bars_country = sorted_bars['name'].iloc[0]

sorted_pop = flags.sort_values('population', ascending = [0])
highest_population_country  = sorted_pop['name'].iloc[0]

## 2. Calculating probability ##

total_countries = flags.shape[0]
orange_count = flags[flags['orange'] == 1]
orange_probability = len(orange_count)/ flags.shape[0]

stripe_count = flags[flags['stripes'] > 1]
stripe_probability = len(stripe_count)/flags.shape[0]
